...
auto async_main(timed_run_loop_scheduler scheduler) -> stdexec::sender auto
{
  ...
  auto deadline = t0 + 500ms;
  auto print_at_deadline = print_tid("A", scheduler.schedule_at(deadline));
  auto jt_scheduler_100ms =
      print_tid("C", jt_scheduler.schedule_after(100ms));
  return print_tid("D",
                   exec::when_any(print_at_deadline, print_at_deadline,
                                  print_at_deadline, print_at_deadline,
                                  print_at_deadline, jt_scheduler_100ms));
}
...
int main()
{
  ...
  stdexec::sender auto drive = stdexec::then(
      stdexec::get_stop_token(), [&](auto token)
      { run_loop.run(token); });
